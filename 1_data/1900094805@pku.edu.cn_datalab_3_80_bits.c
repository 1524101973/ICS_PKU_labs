...
int float_f2i(unsigned uf) {
  ...
  1312817152  -> 3000 0000
  4e400000
  
  0 100 11100 100 0000
  */
  int sign = uf >> 31; // neg 0xffffffff pos 0  //1
  int exp = (uf & 0x7f800000) >> 23;//2
  int frac = uf & 0x7fffff;//1
  int E = 0;
  if(exp == 0xff) return 0x80000000u;//2
  else if(exp < 127) return 0;//2
  exp -= 127;//2
  if(exp >= 31) return 0x80000000u;//2
  else if(exp >= 23) frac = frac << (exp - 23);//4
  else frac = frac >> (23 - exp);//3
  E = (1 << exp) + frac;//2
  if(!sign) return E;//1
  else return ~E + 1;//2
}
