...
int byteSwap(int x, int n, int m) {
  int tmp1 = 0xff << (n << 3); // cnt = 2
  int tmp2 = 0xff << (m << 3);// cnt = 2
  int tmp3 = ~(tmp1 + tmp2) & x; // cnt = 3
  int tmp4 = ((x & tmp1) >> (n << 3) << (m << 3)) & tmp2; // cnt = 6
  int tmp5 = ((x & tmp2) >> (m << 3) << (n << 3)) & tmp1; // cnt = 6
  int tmp6 = x & (~(!(n ^ m)) + 1); // cnt = 2
  return ((tmp3 + tmp4 + tmp5) & (~(!!(n ^ m)) + 1)) | tmp6;// 
}
/* 
...
int logicalShift(int x, int n) {
  int tmp1 = (0x7fffffff & x) >> n;
  int tmp2 = (((0x80000000 & x) >> 31) & 0x1) << 16 >> ((n)>>1) << 15 >> ((n+1)>>1);
  //dont know how to do - 
  int ans = tmp1 + tmp2;
  return ans;
}
...
//     = 0011 0001 1110 0000
    ...
    int tmp2 = x & ((x >> 1) & 0x7fffffff); // cnt = 3
...
int countTrailingZero(int x){
    int tmp_x = x; 
    int reverse_x = ~x; //cnt = 1
    int tmp1 = reverse_x & (~reverse_x + 1);//the first 0 of x // cnt = 3
    int tmp2 = tmp_x & (~tmp_x + 1);//the first 1 of x // cnt = 3
    
    int tmp3 = tmp2 + 0xffffffff; // cnt = 1
    int mask1 = 0x55555555;
    int mask2 = 0x33333333;
    int mask3 = 0x0f0f0f0f;
    int mask4 = 0x00ff00ff;
    int mask5 = 0x0000ffff;
    tmp3 = (tmp3 & mask1) + ((tmp3 >> 1) & mask1); // cnt = 4
    tmp3 = (tmp3 & mask2) + ((tmp3 >> 2) & mask2); // cnt = 4
    tmp3 = (tmp3 & mask3) + ((tmp3 >> 4) & mask3); // cnt = 4
    tmp3 = (tmp3 & mask4) + ((tmp3 >> 8) & mask4); // cnt = 4
    tmp3 = (tmp3 & mask5) + ((tmp3 >> 16) & mask5); // cnt = 4
    //cnt 28   12
    return tmp3 & (~(!(x & 0x1)) + 1);//cnt = 5
}
/* 
...
//we should understand two-complement calculation
    int tmp1 = ((0x80000000 & x) >> 31) & 0x1; //cnt = 3
    int tmp2 = !!(((x >> n) << n) ^ x);// cnt = 5
    return (x >> n) + (tmp1 & tmp2);// cnt = 3
...
*/
  int exception = !((!(x ^ 0x7fffffff)) & (!(y ^ 0x80000000))); //cnt = 6
  //is = exception = !(!(0) & !(0)) = 0
...
*/
    ...
    int flag1 = ~(!((x >> 31) & 0x1)) + 1;//if pos 0xffff ffff neg 0   cnt = 5
    ...
    //pos pos
    return (flag1 & flag2 & result2) | (flag1 & flag2_1 & 0x7fffffff) | (flag1_1 & flag2 & result2) | (flag1_1 & flag2_1 & 0x80000000);
}
/* 
...
unsigned float_i2f(int x) {
  ...
  exp = 31 - cnt;//1
  frac = tmp >> 9;//1
  if((tmp & 0x1ff) > 0x100) rounding = 1;//go up // 4
  else if((tmp & 0x3ff) == 0x300) rounding = 1;//even go up //4
